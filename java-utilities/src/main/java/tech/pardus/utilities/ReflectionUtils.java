...
public class ReflectionUtils {
  ...
  private static void evaluateThreadElements(Entry<Thread, StackTraceElement[]> entry)
      throws ClassNotFoundException {
    for (var element : entry.getValue()) {
      try {
        ...
        if (StringUtils.isNotBlank(stackClass) && StringUtils.contains(stackClass, "$")) {
          stackClass =
              StringUtils.substring(stackClass, 0, StringUtils.lastIndexOf(stackClass, "$"));
        }
          ...
          new BufferedReader(
              new InputStreamReader(
                  ReflectionUtils.class
                      .getClassLoader()
                      .getResourceAsStream("excluded_packet_initials")))) {
  ...
  private static boolean isNotReserved(String name) {
    var control =
        getReservedClasses()
            .stream()
            .filter(t -> StringUtils.startsWith(name, t))
            .findFirst()
            .map(x -> Boolean.FALSE)
            .orElse(Boolean.TRUE);
