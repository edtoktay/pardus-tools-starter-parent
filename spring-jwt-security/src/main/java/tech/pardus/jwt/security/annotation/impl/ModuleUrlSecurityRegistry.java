...
public class ModuleUrlSecurityRegistry {
  ...
  public void registerNewUrl(
      RequestMethod method, String url, String privilege, List<String> roles, boolean isSecured) {
    if (urlRegistry == null) {
      urlRegistry = new HashMap<>();
    }
  ...
  private EndPointModel getEndPoint(RequestMethod method, String url) {
    ...
    return CollectionUtils.isEmpty(urlRegistry.get(method))
        ? null
        : urlRegistry
            .get(method)
            .stream()
